...
@cuda.jit
def nearest_neighbor_kernel(data, output, lon_min, lat_min, lon_max, lat_max, size, cmap, rows, cols, is_re, flags):
    x, y = cuda.grid(2)
    if x >= size or y >= size:
        return
    ...
    RE_VAL, LAT_TS, LON_0 = 6370040.0, 60.0, 10.0
...
def bilinear_kernel(data, output, lon_min, lat_min, lon_max, lat_max, size, cmap, rows, cols):
    ...
    if 0 <= i0 < (rows-1) and 0 <= j0 < (cols-1):
        ...
        val = (v00 * (1.0 - di) * (1.0 - dj) + v01 * (1.0 - di) * dj +
               v10 * di * (1.0 - dj) + v11 * di * dj)
...
class CudaRenderer(RenderingProvider):
    ...
    def render(self, data: np.ndarray, tile_bounds: tuple, product: str, flags: np.ndarray, size: int, interpolation: str = "nearest") -> Image.Image:
        ...
        if interpolation == "bilinear" and product.upper() != "RE":
            bilinear_kernel[blockspergrid, threadsperblock](
                data_gpu, output_gpu, lon_min, lat_min, lon_max, lat_max, size, self.cmap_gpu, rows, cols
            )
        else:
            is_re = (product.upper() == "RE")
            nearest_neighbor_kernel[blockspergrid, threadsperblock](
                data_gpu, output_gpu, lon_min, lat_min, lon_max, lat_max, size, self.cmap_gpu, rows, cols, is_re, flags_gpu
            )
        
        res = Image.fromarray(output_gpu.copy_to_host(), 'RGBA')
        logger.debug("render_microbench", duration=round(time.perf_counter() - t_start, 4), size=size, provider="numba")
        return res
